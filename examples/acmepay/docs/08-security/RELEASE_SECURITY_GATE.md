...
One High finding remains open: session tokens for the approver role are currently valid for 24 hours rather than the target 4-hour lifetime defined in the security baseline, because the shorter-lived token causes noticeable friction in the mobile-web approval flow. Risk-accepted for the pilot cohort only (5 known, contractually bound customers, MFA still mandatory for approvers) with a hard requirement to fix before the security gate can pass for general availability. Tracked in docs/12-planning/DECISION_LOG.md as DEC-004.
